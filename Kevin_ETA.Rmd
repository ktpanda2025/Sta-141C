...
artist_trim_data = Spotify_charts %>%
  filter(artist_names == "Peso Pluma")
```
...
size_input = "total_weeks"
ggplot(data = unique_combined_df, aes_string(y = y_input, x = x_input, size = size_input, color = size_input)) +
  geom_point(alpha = 0.7) +
  scale_size_continuous(range = c(3, 10)) +  # Adjust size range
  scale_color_viridis_c() +  # Add color scale
  labs(y = y_input, x = x_input, size = size_input, color = size_input) +
  theme_minimal()
```
```{r}
```
